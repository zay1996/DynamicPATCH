...
class TransitionAnalysis:
    ...
    def identify(self):
        ...
        ### APPEARING ###
        # gain patch connect to zero final presence 
        app_ind = list(g_fp_counts[g_fp_counts == 0].index)
        pattern[np.isin(self.gainpatchlabels,app_ind)] = 1     
        ### MERGING ###
        ## gain patch connect to at least (or exactly?) one final patch and more than one initial patch 
        ...
        ### FILLING ###
        ## connect to one initial presence and at least (or exactly?) one final presence, and connect to 0 final (?) exterior absence
        ...
        ### DISAPPEARING ###
        ## loss patch connect to zero initial presence
        dis_ind = list(l_ip_counts[l_ip_counts == 0].index)
        pattern[np.isin(self.losspatchlabels,dis_ind)] = 5 
        
        ### SPLITTING ###
        ## loss patch connect to one initial presence and more than one final presence 
        ...
        ### PERFORATING ###
        ## connect to one final presence and [one initial presence], and connect to 0 initial (?) exterior absence 
